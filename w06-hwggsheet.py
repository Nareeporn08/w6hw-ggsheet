...
# Page configuration
st.set_page_config(page_title="ระบบจัดการร้านเครื่องดนตรี",page_icon="🛒",layout="wide")
...
def show_dialogAddProd():
    ...
    cole = st.columns([1, 3,],vertical_alignment="top")
    qty = cole[0].text_input("จำนวนสินค้า")
    brand = cole[1].text_input("แบรนด์")
    ...
    if st.button("เพิ่มสินค้า",type="primary",key="add_product"):
        if name and price > 0 and image_url:
            db.save_product(id,name, brand, price, image_url,qty)
            st.rerun()
        else:
...
def show_dialogEditProd(row_idx,product):
    ...
    if product:
        with st.form(key=f"edit_form_{product['id']}"):
            ...
            edit_brand = cols[3].text_input("แบรนด์", value=product['brand'])
            edit_qty = cols[4].text_input("จำนวนสินค้า", value=product['qty'])
            ...
            if st.form_submit_button("บันทึก"):
                db.update_product(row_idx,edit_id, edit_name, edit_price, edit_brand, edit_qty, edit_image_url)
    ...
    if products:
        ...
        for idx, product in enumerate(products): # Iterate over a copy
            with cols[idx % 4]:
                ...
                st.write(f"แบรนด์: {product['brand']}")
...
def order_page():
    ...
    products = db.load_products()
    active_products = [p for p in products if p['qty'] > 0] #กรองสินค้าที่พร้อมขาย
